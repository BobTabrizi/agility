...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    setAdminToken(getStoredAdminToken(code));
    setClientId(getOrCreateClientId(code));
  }, [code]);

  const { state, error, status, self } = useRoomConnection({
    code,
    name: name ?? "",
    adminToken,
    clientId,
  ...
  return <RoomShell state={state} selfId={self.participantId} isAdmin={self.isAdmin} />;
}
